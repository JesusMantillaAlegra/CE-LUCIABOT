window.LUCIA_DATA = {
  "meta": {
    "generado": "2026-08-19 · datos leídos manualmente de HubSpot (paso previo a automatizar)",
    "fuente_correo": "HubSpot Reports Dashboard 6180490 · vista 20862583",
    "fuente_chat": "HubSpot Reports Dashboard 6180490 · vista 20257473",
    "proxima_actualizacion": "pendiente de automatizar (job diario 6:00 a.m. COT, ver INSTRUCTIVO.md)"
  },
  "correo": {
    "kpis": {
      "demanda": 689,
      "gestionados": 1400,
      "escalados": 411,
      "pct_gestion": 21.19
    },
    "por_stage": [
      {
        "stage": "Closed (correos que no requieren respuesta)",
        "count": 1171
      },
      {
        "stage": "Closed (COL_Sup)",
        "count": 161
      },
      {
        "stage": "Cerrados (Consultas API_Sup)",
        "count": 19
      },
      {
        "stage": "Closed (Nómina_Sup)",
        "count": 10
      },
      {
        "stage": "Waiting on contact (COL_Sup)",
        "count": 9
      },
      {
        "stage": "Closed (Payments Sup)",
        "count": 9
      },
      {
        "stage": "API productos WOC (Consultas API_Sup)",
        "count": 6
      },
      {
        "stage": "Verificación de cliente (Tickets Sales)",
        "count": 2
      },
      {
        "stage": "Waiting on us (COL_Sup)",
        "count": 2
      },
      {
        "stage": "Closed (POS_Sup)",
        "count": 2
      },
      {
        "stage": "Follow up (COL_Sup)",
        "count": 2
      },
      {
        "stage": "Gestionados (Retention)",
        "count": 1
      },
      {
        "stage": "Waiting on contact (DOM_Sup)",
        "count": 1
      },
      {
        "stage": "Follow up 1 (POS_Sup)",
        "count": 1
      },
      {
        "stage": "Seguimiento (Retention)",
        "count": 1
      },
      {
        "stage": "Pending (correos que no requieren respuesta)",
        "count": 1
      }
    ],
    "tendencia_semanal": [
      {
        "semana": "2026-07-03",
        "escaladas": 474,
        "gestionadas": 563
      },
      {
        "semana": "2026-07-13",
        "escaladas": 187,
        "gestionadas": 3
      },
      {
        "semana": "2026-08-02",
        "escaladas": 108,
        "gestionadas": 134
      }
    ],
    "csat": {
      "nota": "muestra pequeña — pocas respuestas de encuesta por semana",
      "serie_semanal": [
        {
          "semana": "2026-07-06",
          "respuestas": 4
        },
        {
          "semana": "2026-07-13",
          "respuestas": null
        },
        {
          "semana": "2026-07-20",
          "respuestas": null
        },
        {
          "semana": "2026-07-27",
          "respuestas": 2
        }
      ]
    }
  },
  "chat": {
    "kpis": {
      "demanda": 4409,
      "ingresados_bot": 3914,
      "escalados": 1175,
      "gestionados": 2735,
      "pct_gestion": 69.88,
      "csat_bot": 88.01
    },
    "ingresados_por_version": [
      {
        "version": "COL",
        "count": 311
      },
      {
        "version": "MEX",
        "count": 278
      },
      {
        "version": "DOM",
        "count": 40
      },
      {
        "version": "CRI",
        "count": 80
      },
      {
        "version": "PER",
        "count": 301
      },
      {
        "version": "VEN",
        "count": 303
      },
      {
        "version": "OTHER",
        "count": 0
      }
    ],
    "tiempo_promedio_solucion_min": 102.9,
    "motivos_solicitud": {
      "COL_AC": [
        {
          "motivo": "Configuración Numeración",
          "count": 114
        },
        {
          "motivo": "Traslado de cuenta",
          "count": 71
        },
        {
          "motivo": "Suscripción plan",
          "count": 51
        },
        {
          "motivo": "Conciliar bancos",
          "count": 47
        },
        {
          "motivo": "Saldos iniciales",
          "count": 44
        },
        {
          "motivo": "Pregunta por issue",
          "count": 44
        },
        {
          "motivo": "Reporte de impuestos",
          "count": 41
        },
        {
          "motivo": "Integraciones",
          "count": 38
        }
      ],
      "Payments": [
        {
          "motivo": "Cancelar suscripción",
          "count": 38
        },
        {
          "motivo": "Solicita descuento",
          "count": 28
        },
        {
          "motivo": "Cambio de producto",
          "count": 23
        },
        {
          "motivo": "Suspendido/bloqueado",
          "count": 22
        },
        {
          "motivo": "Retención factura",
          "count": 21
        },
        {
          "motivo": "Abono a la siguiente",
          "count": 18
        }
      ],
      "Countries_AC": [
        {
          "motivo": "Nota crédito electrónica",
          "count": 8
        },
        {
          "motivo": "Administrador XML",
          "count": 8
        },
        {
          "motivo": "Cambio de moneda",
          "count": 7
        },
        {
          "motivo": "Configuración datos",
          "count": 7
        },
        {
          "motivo": "Lentitud al generar",
          "count": 6
        }
      ],
      "Contador": [
        {
          "motivo": "Emisión Documento",
          "count": 12
        },
        {
          "motivo": "Asesoría",
          "count": 9
        },
        {
          "motivo": "Cambio de plan",
          "count": 8
        },
        {
          "motivo": "Facturación electrónica",
          "count": 7
        },
        {
          "motivo": "Pagos recibidos",
          "count": 5
        }
      ],
      "POS": [
        {
          "motivo": "Plantillas de impresión",
          "count": 19
        },
        {
          "motivo": "Listas de precios",
          "count": 12
        },
        {
          "motivo": "Usuario resolvió la duda",
          "count": 12
        },
        {
          "motivo": "Reporte de transacciones",
          "count": 10
        },
        {
          "motivo": "Configuración dispositivo",
          "count": 10
        }
      ],
      "NE": [
        {
          "motivo": "Terminación de contrato",
          "count": 31
        },
        {
          "motivo": "Configuración empleado",
          "count": 24
        },
        {
          "motivo": "Dudas botón liquidación",
          "count": 21
        },
        {
          "motivo": "Configurar deducción",
          "count": 20
        },
        {
          "motivo": "Configuración inicial",
          "count": 15
        }
      ]
    }
  },
  "llamadas": {
    "estado": "pendiente — se retoma con ElevenLabs + Metabase (bi_bot_calls) en otra sesión"
  }
};
