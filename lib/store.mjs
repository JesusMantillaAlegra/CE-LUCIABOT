...
//
// CORREGIDO 05-sep-2026: al conectar la MISMA base que ya usa
// ce-retention-soporte-ops a este segundo proyecto, Vercel no inyectó
// KV_REST_API_URL/TOKEN a secas -- las prefijó con el nombre del recurso de
// storage (acá "CELuciakv_"), ej. CELuciakv_KV_REST_API_URL,
// CELuciakv_KV_REST_API_TOKEN (confirmado mirando Settings → Environment
// Variables del proyecto). Esto pasa cuando el mismo recurso queda conectado
// a más de un proyecto -- Vercel prefija para evitar choques de nombre. Por
// eso, si no aparecen las variables sin prefijo, se busca cualquier variable
// que TERMINE en _KV_REST_API_URL / _KV_REST_API_TOKEN (o el equivalente de
// Upstash) en vez de asumir un nombre fijo. Ojo: existe también un
// *_KV_REST_API_READ_ONLY_TOKEN -- el regex de abajo no lo agarra a propósito
// (termina distinto), porque acá hace falta el token con permiso de escritura.
function buscarVariablePorSufijo(sufijo) {
  const key = Object.keys(process.env).find((k) => k.endsWith(sufijo));
...
function kvConfig() {
  const url =
    ...
    buscarVariablePorSufijo('_KV_REST_API_URL') ||
    buscarVariablePorSufijo('_UPSTASH_REDIS_REST_URL') ||
    buscarVariablePorSufijo('_REDIS_REST_URL');
  const token =
    process.env.KV_REST_API_TOKEN ||
    process.env.UPSTASH_REDIS_REST_TOKEN ||
    buscarVariablePorSufijo('_KV_REST_API_TOKEN') ||
    buscarVariablePorSufijo('_UPSTASH_REDIS_REST_TOKEN') ||
    buscarVariablePorSufijo('_REDIS_REST_TOKEN');
  if (!url || !token) {
    throw new Error(
      'No hay store de Redis conectado. Faltan KV_REST_API_URL / KV_REST_API_TOKEN ' +
      '(o UPSTASH_REDIS_REST_URL / UPSTASH_REDIS_REST_TOKEN, con o sin prefijo del ' +
      'nombre del recurso) en las variables de entorno de este proyecto. ' +
      'Re-desplegar después de conectar la base para que entren.'
    );
  }
...
// Agrega un snapshot al histórico, identificándolo por `id` -- NO por
// `semana_inicio`. CORREGIDO 05-sep-2026: al sembrar el histórico real se vio
// que varios snapshots comparten el mismo `semana_inicio` a propósito (ej.
// "chat-month-2026-08", "correo-month-2026-08" y "llamadas-month-2026-08"
// los tres tienen semana_inicio="2026-08-01", cada uno con su propio canal
// poblado y el resto en null -- es el modelo nuevo de snapshots mensuales por
// canal). Si se identificara por semana_inicio, guardar o reemplazar un
// snapshot podría pisar el objeto de OTRO canal que coincida en fecha. `id`
// sí es único de verdad en todo el histórico (wk-YYYY-MM-DD para el cron
// semanal viejo, correo-month-YYYY-MM / chat-month-YYYY-MM /
// llamadas-month-YYYY-MM para el flujo mensual nuevo, boot-* para los
// snapshots de arranque) -- por eso se usa como clave acá.
export async function guardarSnapshot(snapshot, { force = false } = {}) {
  if (!snapshot?.id) {
    throw new Error('guardarSnapshot: el snapshot necesita un campo id.');
  }
  const historico = await leerHistorico();
  const i = historico.findIndex((s) => s.id === snapshot.id);
