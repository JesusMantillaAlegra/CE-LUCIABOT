  ...
  return {
    ...
    correo: {
      kpis: {
        ...
        // NUEVO 08-sep-2026: antes solo se guardaba flujo.ingresados, y el dashboard
        // calculaba "% de los ingresados" dividiendo el `gestionados`/`escalados` de arriba
        // (owner ACTUAL / propiedad de escalamiento -- otra metodología) entre este
        // ingresados_flujo (owner-history). Como son dos consultas distintas sobre
        // poblaciones de tickets que no son subconjunto una de la otra, las dos tarjetas
        // podían sumar más (o menos) de 100% -- bug reportado por Jesús 08-sep-2026.
        // Guardando también gestionados_flujo/escalados_flujo (misma consulta owner-history
        // que ingresados_flujo) el dashboard puede dividir cada uno entre ingresados_flujo
        // y sí sumar exactamente 100%.
        gestionados_flujo: flujo.gestionados,
        escalados_flujo: flujo.escalados,
      },
...
async function construirSnapshotLlamadas(mes) {
  const { inicio, finExclusivo } = mes;
  const [{ porVersion, motivo_escalamiento }, csat] = await Promise.all([
    llamadasPorVersion(inicio, finExclusivo),
    llamadasCsat(inicio, finExclusivo),
  ]);
  ...
  return {
    ...
    llamadas: {
      kpis: {
        ...
        duracion_prom_seg,
        // CSAT -- NUEVO 22-sep-2026 (pedido Juan Montero, CSDATA-391). Viene del evento de
        // comportamiento "Encuesta de satisfacción CS" (pe6180490_customer_survey_score),
        // filtrado por Agente=Lucía-IVR, escala real 1-5 (top-2-box 4-5 = satisfecho). Ver
        // llamadasCsat() en lib/hubspot.js para el detalle completo (por qué 1-5 y no 1-10,
        // por qué se descarta el resto como ruido de ingesta).
        csat_pct: csat.pct_positivo,
        csat_positivos: csat.positivos,
        csat_total: csat.total,
      },
      por_version: porVersion,
      motivo_escalamiento,
      _nota_cubo_en_vivo: `Calculado en vivo el ${new Date().toISOString()} por lib/cubos.mjs. CSAT: pct_positivo=${csat.pct_positivo}, positivos=${csat.positivos}, total=${csat.total}, descartados_fuera_de_rango=${csat.descartados || 0}${csat.error ? `, ERROR al traer CSAT: ${csat.error}` : ""}.`,
    },
  };
}

// CSAT de Chat -- histórico CONGELADO de Breeze (22-sep-2026, pedido Juan Montero, CSDATA-391).
//
// Investigación en vivo (22-sep-2026, ver diagnosticar_csat_agentes_evento.mjs y
// diagnosticar_csat_chat_ticket_property.mjs) descartó las DOS fuentes que sí sirvieron para
// Llamadas/Correo:
//   1. El evento de comportamiento "Encuesta de satisfacción CS" (pe6180490_customer_survey_score,
//      el mismo que usa llamadasCsat()) -- se listaron TODOS los valores de agent existentes
//      (18142 eventos) y ninguno corresponde a Chat: son colas de IVR (Lucia-IVR, Lucia-IVR-dom,
//      Laura IVR DE, Paula IVR pay/RET, etc.) o nombres de agentes humanos. Este evento no
//      cubre Chat en absoluto.
//   2. La property de ticket clasificacion_encuesta_ces_csat (la que usa correoCsatSemana())
//      -- SÍ aparece en tickets de Chats_Sup (110 casos encontrados sin filtro de fecha), pero
//      CERO con hubspot_owner_id=89503870 (Lucía). Esa encuesta se le manda a agentes humanos
//      de chat, no a conversaciones resueltas por el bot.
// Una tercera fuente quedó documentada (DOCUMENTACION_METRICAS_LUCIA.md, sección "CSAT
// gestionado por bot": objeto feedback_submissions cruzado con conversationId, encuestas
// "Encuesta directa chat...", owner IN Lucía+Isabel) pero ese mismo documento avisa tener
// "bugs conocidos sin corregir" y nunca se implementó en código -- no se usa acá sin
// verificarla en vivo primero (mismo criterio de todo este archivo: no inventar).
//
// Lo único real que SÍ existe: Jesús ya había sacado estos 3 meses a mano vía HubSpot AI
// (Breeze) el 28-ago-2026, guardados en el histórico viejo antes de migrar a cálculo en vivo
// (lucia_dashboard_history.json). Junio ya estaba en KV (protegido por ser mesEstable);
// Julio y Agosto se estaban PERDIENDO porque este cubo los recalcula en vivo en cada corrida
// del cron y csat_bot no estaba implementado acá (volvía a null). Se restauran como valores
// CONGELADOS -- no se recalculan, no van a cambiar con nueva data de HubSpot -- con el mismo
// alcance limitado que ya tenían documentado: CSAT cubre SOLO Contador/POS/Nómina (COL y
// AC/Countries no registran respuestas de encuesta en este canal), NO es CSAT total de Chat.
// Agosto además es parcial (la consulta de Breeze corrió hasta el día 28, no el mes completo).
// Si en el futuro se confirma una fuente en vivo real (ej. feedback_submissions verificado),
// esta tabla se reemplaza por el cálculo en vivo -- por ahora es lo más honesto que hay.
const CHAT_CSAT_HISTORICO_BREEZE = {
  "2026-06": { positivas: 72, total: 81, pct: 88.9 },
  "2026-07": { positivas: 143, total: 176, pct: 81.3 },
  "2026-08": { positivas: 109, total: 128, pct: 85.2 }, // parcial: Breeze corrió esta consulta hasta el 28-ago-2026
};
...
async function construirSnapshotChat(mes) {
  ...
  const csatCongelado = CHAT_CSAT_HISTORICO_BREEZE[mes.id] || null;
  return {
    ...
    chat: {
      kpis: {
        ...
        gestionados,
        escalados,
        csat_bot: csatCongelado ? csatCongelado.pct : null,
      },
      csat_bot_detalle: csatCongelado ? { positivas: csatCongelado.positivas, total: csatCongelado.total, pct: csatCongelado.pct } : null,
      ingresados_por_version: [],
      tiempo_promedio_solucion_min: null,
      motivos_solicitud: {},
      _nota_cubo_en_vivo: `Calculado en vivo el ${new Date().toISOString()} por lib/cubos.mjs. demanda = tickets creados en la pipeline Chats_Sup (125444762) en el período. gestionados = de esos, hubspot_owner_id=89503870 (Lucía) -- validado 07-sep-2026: coincide al 100% (1105/1105) con categoria_bot_lucia=gestionadas sobre una muestra real de 2000 tickets de agosto. escalados = de esos, inbox_id_objetivo con valor (propiedad real "Escalamiento inbox objetivo", única señal de escalado poblada de forma no trivial en Chat -- chat_transferido y escalamiento_lucia_email salieron vacíos). ingresados_bot = gestionados+escalados (aproximación). ingresados_por_version, tiempo_promedio_solucion_min y motivos_solicitud NO están implementados en vivo todavía -- se dejan null/vacíos a propósito (no inventar un número sin fuente confirmada), index.html los muestra como "no disponible". csat_bot${csatCongelado ? ` = ${csatCongelado.pct}% (valor CONGELADO de Breeze 28-ago-2026, NO recalculado en vivo -- cubre solo Contador/POS/Nómina, no es CSAT total de Chat; ver nota grande junto a CHAT_CSAT_HISTORICO_BREEZE)` : ` = null (no hay valor congelado de Breeze para este mes ni fuente en vivo confirmada -- se investigaron 2 fuentes de HubSpot el 22-sep-2026 y ninguna sirvió, ver nota grande junto a CHAT_CSAT_HISTORICO_BREEZE)`}.`,
    },
  };
}

// Meses de inicio reales de cada canal (Jesús, 05-sep-2026 y 07-sep-2026, CORREGIDO
// 22-sep-2026): Correo y Llamadas arrancan en julio 2026; Chat arranca en MAYO 2026.
//
// CORREGIDO 22-sep-2026 (Jesús): Chat arrancaba antes en febrero 2026, con la decisión de
// dejar feb/mar tal cual los devolviera HubSpot en vivo (ver historial abajo). Esa decisión
// se revirtió -- Jesús pidió dejar el canal Chat solo desde mayo 2026 en adelante, feb/mar/
// abr quedan fuera del rango que este cubo genera. OJO: mesesEnRango() vuelve a generar
// cualquier snapshot dentro de [CHAT_INICIO, hoy) en cada corrida del cron -- si CHAT_INICIO
// se mueve hacia atrás otra vez, esos meses reaparecen solos aunque se hayan borrado a mano
// de KV con /api/limpiar-historico (ya pasó una vez: los snapshots de feb/mar que se habían
// borrado por estar en 0/0 volvieron a aparecer en la corrida siguiente del cron). Si se
// vuelve a achicar/agrandar este rango, hay que borrar a mano en KV los meses que queden
// fuera, con /api/limpiar-historico?ids=... -- moverlo en el código NO borra lo que ya está
// guardado.
//
// Historial (por qué antes arrancaba en feb): para feb/mar, hubspot_owner_id=89503870 /
// inbox_id_objetivo devuelven 0 gestionados/0 escalados en vivo -- se confirmó con
// diagnosticar_gestionados_por_mes.mjs que no es un bug, es lo que HubSpot realmente
// tiene bajo esta definición para esos meses (el tracking por owner/inbox no llega tan
// atrás, aunque el bot sí operaba y resolvía casos reales por entonces -- entonces se
// llamaba "Isabel", owner id 76066940, ver DOCUMENTACION_METRICAS_LUCIA.md). Abril tampoco
// se salva del todo (144 gestionados de 8905 de demanda -- tracking parcial, no confiable).
export const CORREO_LLAMADAS_INICIO = "2026-07-01";
export const CHAT_INICIO = "2026-05-01";
