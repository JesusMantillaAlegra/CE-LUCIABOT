...
//
// NUEVO 22-sep-2026 (llamadasCsat, más abajo): además pega a /events/v3/events (Custom
// Behavioral Events), que es una API distinta a los objetos CRM de arriba y puede requerir
// habilitar un scope propio para el Private App (algo como "business-intelligence" o el
// scope de "Analytics y eventos" en el panel de scopes de HubSpot -- no confirmado en vivo
// todavía). Si el token no tiene ese scope, llamadasCsat() no revienta el cron: devuelve
// { promedio: null, ... } y lo deja registrado en el _nota_cubo_en_vivo del snapshot para
// poder detectarlo. Si el CSAT de Llamadas sale siempre null/"no disponible" en el
// dashboard después de desplegar esto, revisar los scopes del Private App primero.
//
...
const CHATS_SUP_PIPELINE_ID = "125444762"; // pipeline "Chats_Sup" -- confirmado con Breeze 07-sep-2026, cubre ~99% de los tickets source_type=CHAT
...
// ---------- Chat: Demanda vía TICKETS (Chats_Sup) ----------
// NUEVO 07-sep-2026: alternativa a chatDemandaCount() de arriba, validada con Breeze
// contra datos reales de agosto 2026. En vez de paginar la Conversations API por las 5
// bandejas (lento, sujeto a los problemas de "sort" ya documentados en
// searchThreadsInRange), cada conversación de Chat que llega a soporte genera un ticket
// en el pipeline "Chats_Sup" (id 125444762) -- igual que Correo usa COL_Sup. Validado:
// en agosto 2026, Chats_Sup tuvo 8357 tickets vs. 8407 tickets con source_type=CHAT en
// TOTAL (o sea, cubre ~99% del volumen operativo real de chat) y vs. 7278 conversaciones
// contadas por chatDemandaCount() (bandejas específicas, sin ruido) -- la diferencia
// (~15%) es population algo más amplia (Chats_Sup incluye alguna bandeja extra como
// "Payments chat" y "Unassigned" que chatDemandaCount() no cuenta). Mismo patrón que
// Correo: se documenta el alcance en vez de perseguir un número idéntico entre dos
// definiciones distintas. Pendiente de decidir cuál de las dos queda como Demanda oficial
// del tablero -- por ahora se deja como función separada, no reemplaza chatDemandaCount().
async function chatDemandaTicketsCount(semanaInicio, semanaFinExclusive) {
  const filters = [
...
// ---------- Chat: Ingresados / Gestionados / Escalados ----------
// DESBLOQUEADO 07-sep-2026, con datos reales (no vía Breeze -- Breeze no puede ejecutar
// consultas en vivo contra la Conversations API, solo ve el esquema del portal).
//
// Se inspeccionó un thread REAL de agosto 2026 (11060016445, ticket "Test escalamiento
// ticekts" en Chats_Sup) vía /conversations/v3/conversations/threads/{id}/messages +
// /conversations/v3/conversations/actors/{id}. Hallazgo clave: el actor que le responde
// al visitante es `A-89503870`, tipo AGENT, nombre "Lucía Pérez", email
// apps-support@alegra.com -- es decir, EL MISMO owner id (89503870, ya usado en Correo)
// identifica a Lucía en Chat también. El actor tipo BOT que sí aparece (`B-...`, nombre
// "Alegra") es un bot de bienvenida nativo de HubSpot, NO es Lucía -- Lucía corre como
// una integración externa que actúa vía un "agent seat" fijo, no como chatflow nativo.
//
// Validado sobre 2000 tickets reales de Chats_Sup de agosto 2026
// (diagnosticar_chat_escalados.mjs, 07-sep-2026):
//   - hubspot_owner_id=89503870  ==  categoria_bot_lucia="gestionadas"  (1105 de 1105,
//     coinciden al 100% -- son la misma señal, se usa owner_id por consistencia con Correo).
//   - chat_transferido y escalamiento_lucia_email: 0 tickets con valor en la muestra --
//     NO sirven para Chat (escalamiento_lucia_email es específico de Correo).
//   - `inbox_id_objetivo` (label real: "Escalamiento inbox objetivo"): 104 tickets con
//     valor -- es la señal real de escalado en Chat, análoga a escalamiento_lucia_email
//     en Correo. `escalation_target_agent` también existe pero casi vacío (2/2000), se
//     descarta por ahora.
//
// Definiciones (mismo patrón que Correo):
//   Ingresados  = todos los tickets creados en Chats_Sup en el período (== chatDemandaTicketsCount).
//   Gestionados = de esos, los que tienen hubspot_owner_id = Lucía (89503870).
//   Escalados   = de esos, los que tienen inbox_id_objetivo poblado (HAS_PROPERTY).
async function chatIngresadosGestionadosEscalados(semanaInicio, semanaFinExclusive) {
  const baseFilters = [
    { propertyName: "hs_pipeline", operator: "EQ", value: CHATS_SUP_PIPELINE_ID },
    ...dateRangeFilters("createdate", semanaInicio, semanaFinExclusive),
  ];
  const [ingresados, gestionados, escalados] = await Promise.all([
    countTickets([{ filters: baseFilters }]),
    countTickets([{ filters: [...baseFilters, { propertyName: "hubspot_owner_id", operator: "EQ", value: LUCIA_OWNER_ID }] }]),
    countTickets([{ filters: [...baseFilters, { propertyName: "inbox_id_objetivo", operator: "HAS_PROPERTY" }] }]),
  ]);
  return { ingresados, gestionados, escalados };
}
...
// ---------- Llamadas: CSAT (evento de comportamiento "Encuesta de satisfacción CS") ----------
// NUEVO 22-sep-2026, a pedido de Juan Montero (Linear CSDATA-391, comentarios 10-sep y
// 15-sep-2026, referenciando el panel HubSpot 6180490/view/21499085 "Bots de voz -
// elevenlabs"). A diferencia de todo lo demás en este archivo, el CSAT de Llamadas NO es
// una propiedad de un objeto CRM (Ticket/Call) -- es un evento de comportamiento
// personalizado de HubSpot (Custom Behavioral Event), API completamente distinta
// (events/v3), que se dispara vía API externa (Configuración: "Manual" en HubSpot) cada
// vez que un usuario califica una llamada.
//
// Verificado en vivo el 22-sep-2026 en el panel de HubSpot (Gestión de eventos ->
// "Encuesta de satisfacción CS"):
//   - Nombre interno del evento: pe6180490_customer_survey_score
//   - Objeto principal: Contact
//   - Propiedades usadas acá: "agent" (Agente que atendió la llamada) y "surveyscore"
//     (Calificación Encuesta, tipo Enumeración -- "la calificación asignada por el
//     usuario").
//
// ESCALA REAL (corregido 22-sep-2026, ver diagnosticar_csat_llamadas_distribucion.mjs):
// la primera versión de esto asumía una escala 1-10 (calcada del "promedio 7.86" que
// mostraba el resumen con IA del reporte de referencia de HubSpot). Al bajar la
// distribución real de eventos con agent=Lucía-IVR (528 eventos, jul-sep 2026) salió que
// el 82% (405/495 de los valores 0-10) son exactamente "5", con una cola larga hacia abajo
// (0,1,2,3,4) y prácticamente nada arriba de 5 salvo ruido de ingesta claro (11, 12, 16,
// 20...1000). Eso NO es la forma de un promedio 1-10 con ruido -- es exactamente la forma
// esperada de una escala 0-5 (tipo "estrellas") donde la mayoría califica el máximo. Con
// esa lectura, "5" es una calificación real y positiva, no un valor por defecto.
// Decisión de Jesús 22-sep-2026: asumir la lectura positiva (escala 1-5, top-2-box 4-5 =
// satisfecho) en vez de descartar el "5" como sospechoso. La escala es 1-5 (no 0-5): el
// "0" (6 casos en la distribución real) se trata como el resto del ruido fuera de escala,
// no como una calificación baja válida -- un selector de estrellas típico no ofrece "0".
// Igual que en Correo/Chat, se muestra como % de respuestas positivas, no como promedio
// crudo.
//   - El evento tiene 113 errores de ingesta marcados en HubSpot, y valores claramente
//     fuera de escala (0, 11, 12, 16, 20, 21, 25, 27, 50, 57, 60, 65, 70, 87, 90, 100, 110,
//     125, 420, 500, 600, 1000 vistos en la distribución real) -- se descartan
//     explícitamente (SURVEYSCORE_MIN/MAX = 1-5) en vez de dejarlos ensuciar el cálculo.
//     Los descartados se cuentan aparte (csat.descartados) para poder ver en el dashboard
//     si el % de ruido crece y hay que revisar esto de nuevo.
//   - No usa searchCallsAll/searchTicketsAll (esos pegan a /crm/v3/objects/.../search) --
//     esto pega a /events/v3/events, con paginación por cursor `after` igual que las demás.
const LLAMADAS_CSAT_EVENT_TYPE = "pe6180490_customer_survey_score";
const LLAMADAS_CSAT_AGENTE = "lucia-ivr"; // comparado sin tildes/mayúsculas contra properties.agent
const SURVEYSCORE_MIN = 1;
const SURVEYSCORE_MAX = 5;
const SURVEYSCORE_POSITIVO_DESDE = 4; // top-2-box: 4 y 5 cuentan como "satisfecho"

function sinTildes(s) {
  return (s || "").normalize("NFD").replace(/[̀-ͯ]/g, "").toLowerCase().trim();
}

async function fetchBehavioralEventsAll(eventType, occurredAfterISO, occurredBeforeISO) {
  const all = [];
  let after;
  do {
    ...
    const page = await hsFetch(`/events/v3/events?${params.toString()}`);
    all.push(...(page.results || []));
    after = page.paging?.next?.after;
    if (after) await sleep(150);
  } while (after);
...
async function llamadasCsat(semanaInicio, semanaFinExclusive) {
  const occurredAfterISO = `${semanaInicio}T00:00:00.000Z`;
  const occurredBeforeISO = `${semanaFinExclusive}T00:00:00.000Z`;
  let eventos;
  try {
    eventos = await fetchBehavioralEventsAll(LLAMADAS_CSAT_EVENT_TYPE, occurredAfterISO, occurredBeforeISO);
  } catch (e) {
    // No se inventa un número: si el token no tiene el scope de eventos de comportamiento
    // (u otro error de la API), se devuelve null y el dashboard lo muestra como "no
    // disponible" -- igual criterio que csat_bot:null en construirSnapshotChat.
    return { pct_positivo: null, positivos: 0, total: 0, descartados: 0, error: e.message };
  }
  const deLucia = eventos.filter((ev) => sinTildes(ev.properties?.agent) === LLAMADAS_CSAT_AGENTE);
  let positivos = 0, total = 0, descartados = 0;
  deLucia.forEach((ev) => {
    const v = Number(ev.properties?.surveyscore);
    if (Number.isNaN(v) || v < SURVEYSCORE_MIN || v > SURVEYSCORE_MAX) { descartados++; return; }
    total++;
    if (v >= SURVEYSCORE_POSITIVO_DESDE) positivos++;
  });
  return { pct_positivo: total ? +((100 * positivos) / total).toFixed(1) : null, positivos, total, descartados };
}
